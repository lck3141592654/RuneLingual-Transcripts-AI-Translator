...
## 最近更新 (24/6/2026)，更新詳情請看 [Changelog](Changelog.md)
- **新增校對管線**：新增 `proofreader.py` + `batch_proofread.py`，支援 LLM 雙輪評估、潤色與重譯保護
- **移除 response_format**：所有 API 調用（主翻譯與校對）改用 Prompt 直接要求原始 JSON 陣列，提高相容性
- 共用 `run_worker_pool()` 和 `get_relevant_glossary()` 提取至 `llm_translator.py`，供翻譯與校對共用
...
```
校對腳本
batch_proofread.py → 逐工作表處理：
① Phase 2: LLM 雙輪評估（R1+R2 混合批次池）
② Phase 3: LLM 潤色（最多 3 輪）
③ Phase 4a: 重譯保護
...
**Q：校對可以處理多個工作表嗎？**
<br>A：目前只支援單工作表處理。若需要校對多個工作表，請逐表執行校對腳本。
