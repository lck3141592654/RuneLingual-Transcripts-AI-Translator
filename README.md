...
## 最近更新 (9/8/2026)，更新詳情請看 [Changelog](Changelog.md)
- **空格檢查修正**：不再破壞英文文本，只處理英文半形括號與中文標點空格；中英混排空格（如「屠龙者 I 任务」）不再誤報
- **術語匹配重寫**：新增 `find_term_spans()`，只接受術語原樣或真正的複數變形；News 不再誤配 new、The Face 不再誤配 face；單數詞不再誤配複數術語（bus ≠ Buses）
- **ADD_LIST 同字條目修正**：RuneLingual、OSRS 等「譯文 = 原文」的術語不再誤報未翻譯
- **字串 "nan"/"nat"/"none" 修正**：不再被翻譯階段漏掉，三處判斷行為一致
- **重譯提示詞瘦身**：每批只帶該批實際出現的術語，避免超 token
- **校對續傳強化**：審查記錄存完整內容、R2 缺失預設「严重」、完成後清除 session
- **術語庫防呆**：english 為空的列不再變成 'nan' 條目
- **效能優化**：21,000+ 術語大表掃描從約 18 秒降至約 2.5 秒
...
- **checkpoint 原子寫入**：所有 checkpoint 統一以 .tmp → fsync → rename 原子寫入
- **四色審查報告**：合併審查報告補上空格問題綠色標記
...
- **多工作表並行**：所有工作表在同一階段內並行，階段間同步；共用單一 API 批次池，429 冷卻狀態全 run 一致
- **自動審查**：四層檢查（術語 / 佔位符 / 未翻譯 / 空格），產出彩色標示的審查報告
- **多 API 支援**：支援同時使用多個 API Key（主/副分類），輪流分配任務，自動處理 429 限流（冷卻/永久停用），最大化翻譯吞吐量
- **自動化校對流程**：LLM 雙輪評估交叉驗證 + 最多 3 輪潤色 + 重譯保護，產出彩色標記的校對報告
- **基於術語校對**：校對過程中強制套用術語庫，避免 LLM 在潤色時偏離正確術語
...
| `enforcer.py` | **四層強制檢查**（術語/佔位符/未翻譯/空格），附腳本預處理，自動修正+產出彩色審查報告 |
| `shared_pool.py` | **共享批次池** — 整個執行共用一份 API 配置、並發與 429 冷卻狀態 |
...
> `workplace/` 目錄包含以下子目錄：
> - `_checkpoint/` — 翻譯續傳暫存，各工作表擁有獨立子目錄（`{sheet_name}/`），翻譯完成後自動刪除
...
③ llm_translator.py AI 批次翻譯（全部工作表並行）
④ enforcer.py      四層強制檢查 + 腳本預處理 + 重譯（全部工作表並行）
```
```
校對腳本
batch_proofread.py → 多工作表並行（階段間同步）：
① Phase 2: LLM 雙輪評估（全部工作表並行）
② Phase 3: LLM 潤色（最多 3 輪，全部工作表並行）
...
### 使用方法：主翻譯
將翻譯目標 Excel 放入 `workplace/` 後執行 `batch_translate.py`。
...
Step 5：確認執行          → 開始翻譯（模板比對 → LLM 批次翻譯 → 術語強制後處理），完成後輸出翻譯檔與審查報告
```
...
### 使用方法：校對
將已翻譯完成的 Excel 放入 `workplace/` 後執行 `batch_proofread.py`。
> ⚠️ **注意**：校對管線支援**多工作表**處理（各階段內並行，階段間同步）。
...
Step 4：選擇校對範圍      → 選取要校對的條目範圍
Step 5：確認執行          → 開始校對（Phase 2→3→4a→4b→報告）
```

### 執行後產出
| 檔案 | 說明                                      |
|------|-----------------------------------------|
| `{來源檔名}_proofread_output.xlsx` | **校對後的完整 Excel**                        |
...
若翻譯/校對過程中因網路斷線、API 錯誤、或人手中斷（Ctrl+C），腳本支援從中斷處繼續：
...
5. 選擇 `Y` → 自動載入術語庫設定、還原已翻譯/校對條目，從中斷處繼續
...
| `API_TIMEOUT` | 3600 | 所有 LLM 請求的統一逾時秒數（api_config.py） |
| `PERMANENT_DISABLE_AFTER` | 2 | 累計第 N 次 429 後永久停用該 API（api_config.py） |
...
| 翻譯數量 | 平均成本（主翻譯）      | 主翻譯耗時（三次測試） | 平均成本（校對） | 校對耗時（三次測試） |
|---------|---------------|----------------|----------|----------------|
| 500 條 | 0.01-0.02 USD | 329s, 467s, 353s |0.02 USD| 350s, 387s, 461s |
| 2000 條 | 0.06-0.07 USD | 801s, 746s, 696s |0.07-0.08 USD| 912s, 1243s| 
| 5000 條 | 0.14 USD      | 2084s, 1465s, 2170s |
| 75000 條 | 2.39 USD | 8 hours | | |
...
**Q：校對時出現 `[APIx] ⚠️ 完成率 40%（4/10）低於 75%，重試`**
